...
import 'package:edu_rating_app/pages/admin/study/study_edit.dart';
import 'package:edu_rating_app/utils/common_toast.dart';
import 'package:flutter/Material.dart';

class InfoStudyManagePage extends StatefulWidget {
  const InfoStudyManagePage({Key? key}) : super(key: key);

  @override
  State<InfoStudyManagePage> createState() => _TeachManagePageState();
}

class _TeachManagePageState extends State<InfoStudyManagePage> {
  //管理当前widget的state
  final GlobalKey<FormState> formKey = GlobalKey<FormState>();
  SourceData myDS = SourceData();
  StudyModel formData = StudyModel();
  int rowsPerPage = 8;
  ...
  _edit({StudyModel? studyModel}) {
    showDialog(
      context: context,
      builder: (BuildContext context) => Dialog(
        child: StudyEdit(
          teachModel: studyModel,
        ),
      ),
    ).then((v) {
      if (v != null) {
          ...
          CryButton(
            ...
            onPressed: myDS.selectedCount != 1
                ...
                : () {
                    ...
                    StudyModel studyModel = myDS.dataList.firstWhere((v) {
                      return v.selected!;
                    });
                    _edit(studyModel: studyModel);
                  },
                          ...
                          actions: <Widget>[
                            ...
                            TextButton(
                              ...
                              onPressed: () async {
                                ...
                                var result = await StudyApi.removeByIds(ids);
            //                     StudyApi.saveOrUpdate(studyModel!.toMap()).then((res) {
...
class SourceData extends DataTableSource {
  ...
  loadData() async {
    ...
    ResponseBodyApi responseBodyApi =
        await StudyApi.page(requestBodyApi.toMap());
    page = PageModel.fromMap(responseBodyApi.data);

    //解析得到返回的的数据表
    dataList = page.records.map((e) {
      StudyModel studyModel = StudyModel.fromMap(e);
  ...
  DataRow? getRow(int index) {
    ...
    StudyModel userModel = dataList[dataIndex];
    ...
    return DataRow.byIndex(
        ...
        cells: [
          ...
          DataCell(TextButton( onPressed: () {
              Navigator.pushNamed(context, '/studyEval/view?courseID=${userModel.courseID}&courseName=${Uri.encodeComponent(userModel.courseName!)}',);
              }, child: Text('详情'),)),
        ]);
  }
